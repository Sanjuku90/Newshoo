...
import { CheckCircle2, XCircle, ExternalLink, CheckSquare } from "lucide-react";
...
export default function AdminDeposits() {
  ...
  const pendingItems = items.filter((d: any) => d.status === "pending");
  ...
  const toggleAll = () => {
    ...
    else setSelected(new Set(pendingItems.map((d: any) => d.id)));
  };
  ...
  const bulkApprove = async () => {
    ...
    setSelected(new Set());
    toast({ title: `${count} dépôt(s) approuvé(s)` });
    refetch();
  };
  ...
  const bulkReject = async () => {
    ...
    setBulkReason("");
    toast({ title: `${count} dépôt(s) rejeté(s)` });
    refetch();
  };

  const handleApprove = (dep: any) => {
    approve.mutate({ id: dep.id }, {
      onSuccess: () => { toast({ title: "Dépôt approuvé !" }); refetch(); },
  ...
  const handleReject = () => {
    reject.mutate({ id: rejectTarget.id, data: { reason } }, {
      onSuccess: () => { toast({ title: "Dépôt rejeté" }); setRejectTarget(null); setReason(""); refetch(); },
      onError: (err: any) => toast({ title: "Erreur", description: err?.data?.error, variant: "destructive" }),
    });
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold mb-1">Dépôts</h1>
        <p className="text-muted-foreground text-sm">Vérifier et approuver les dépôts utilisateurs</p>
      </div>
      ...
      {isLoading ? (
        ...
        <Card className="border-border"><CardContent className="py-12 text-center text-muted-foreground">Aucun dépôt {statusFilter === "pending" ? "en attente" : statusFilter === "approved" ? "approuvé" : "rejeté"}</CardContent></Card>
      ) : (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.map((dep: any) => {
                ...
                return (
                  <tr key={dep.id} className={`border-b border-border/50 transition-colors ${isSelected ? "bg-primary/5" : "hover:bg-secondary/20"}`}>
                    {statusFilter === "pending" && (
                      <td className="py-3 pr-3">
                        <button onClick={() => toggleSelect(dep.id)} className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${isSelected ? "bg-primary border-primary" : "border-border hover:border-primary"}`}>
                    ...
                    <td className="py-3 pr-4 font-semibold text-emerald-400">{dep.amount} USDT</td>
                    ...
                    <td className="py-3 pr-4 text-muted-foreground text-xs">{new Date(dep.createdAt).toLocaleDateString("fr-FR")}</td>
                    <td className="py-3">
                      {dep.status === "pending" && (
                        <div className="flex gap-2">
                          <Button size="sm" variant="outline" className="gap-1 text-emerald-400 border-emerald-400/30 hover:bg-emerald-400/10" onClick={() => handleApprove(dep)}>
      ...
      <Dialog open={!!rejectTarget} onOpenChange={open => { if (!open) setRejectTarget(null); }}>
        <DialogContent className="bg-card border-border">
          <DialogHeader><DialogTitle>Rejeter le dépôt</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">Dépôt de {rejectTarget?.amount} USDT — {rejectTarget?.userFirstName} {rejectTarget?.userLastName}</p>
      ...
      <Dialog open={bulkRejecting} onOpenChange={setBulkRejecting}>
        <DialogContent className="bg-card border-border">
          <DialogHeader><DialogTitle>Rejeter {selected.size} dépôt(s)</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">Cette action va rejeter {selected.size} dépôt(s) sélectionné(s).</p>
            <div className="space-y-2">
