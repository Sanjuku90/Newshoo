...
import { db, usersTable, depositsTable, withdrawalsTable, investmentsTable, plansTable, ticketsTable, ticketMessagesTable, kycDocumentsTable, transactionsTable } from "@workspace/db";
...
router.get("/admin/users", requireAdmin, async (req, res) => {
  ...
  const safeUsers = users.map(u => {
    ...
    return { ...safe, totalInvested: safe.totalInvested ?? "0", createdAt: safe.createdAt?.toISOString() };
  });
...
router.get("/admin/users/:id", requireAdmin, async (req, res) => {
  ...
  res.json({
    ...
    investments: investments.map(i => ({
      id: i.id, planId: i.planId, planName: "Plan", amount: parseFloat(i.amount),
      dailyRate: parseFloat(i.dailyRate), durationDays: i.durationDays,
      startDate: i.startDate?.toISOString(), endDate: i.endDate?.toISOString(),
      status: i.status, totalEarned: parseFloat(i.totalEarned ?? "0"), dailyEarning: 0,
    })),
    deposits: deposits.map(d => ({
      id: d.id, amount: parseFloat(d.amount), status: d.status,
      walletAddress: d.walletAddress, txHash: d.txHash,
      rejectionReason: d.rejectionReason, createdAt: d.createdAt?.toISOString(),
    })),
    withdrawals: withdrawals.map(w => ({
      id: w.id, amount: parseFloat(w.amount), fee: parseFloat(w.fee ?? "0"),
      netAmount: parseFloat(w.netAmount), status: w.status,
      walletAddress: w.walletAddress, rejectionReason: w.rejectionReason,
...
router.get("/admin/deposits", requireAdmin, async (req, res) => {
  ...
  const result = await Promise.all(deposits.map(async (d) => {
    ...
    return {
      ...
      amount: parseFloat(d.amount), status: d.status,
...
router.post("/admin/deposits/:id/approve", requireAdmin, async (req, res) => {
  ...
  if (user) {
    ...
    await db.update(usersTable).set({
      ...
      totalDeposited: (parseFloat(user.totalDeposited ?? "0") + amount).toString(),
      updatedAt: new Date(),
    }).where(eq(usersTable.id, user.id));

    await db.insert(transactionsTable).values({
      userId: user.id, type: "deposit", amount: deposit.amount,
      status: "completed", description: "Deposit approved",
    });
  }
...
router.get("/admin/withdrawals", requireAdmin, async (req, res) => {
  ...
  const result = await Promise.all(withdrawals.map(async (w) => {
    ...
    return {
      ...
      userName: user ? `${user.firstName} ${user.lastName}` : "Unknown",
      amount: parseFloat(w.amount), fee: parseFloat(w.fee ?? "0"),
      netAmount: parseFloat(w.netAmount), status: w.status,
      walletAddress: w.walletAddress, createdAt: w.createdAt?.toISOString(),
    };
  }));
  res.json(result);
});
...
router.post("/admin/withdrawals/:id/approve", requireAdmin, async (req, res) => {
  ...
  if (user) {
    await db.update(usersTable).set({
      totalWithdrawn: (parseFloat(user.totalWithdrawn ?? "0") + parseFloat(updated.amount)).toString(),
    }).where(eq(usersTable.id, user.id));
  }
...
router.post("/admin/withdrawals/:id/reject", requireAdmin, async (req, res) => {
  ...
  if (withdrawal) {
    ...
    if (user) {
      await db.update(usersTable).set({
        mainBalance: (parseFloat(user.mainBalance ?? "0") + parseFloat(withdrawal.amount)).toString(),
      }).where(eq(usersTable.id, user.id));
    }
  }
...
router.get("/admin/kyc", requireAdmin, async (req, res) => {
  ...
  let docs;
  if (status) {
    docs = await db.select().from(kycDocumentsTable).where(eq(kycDocumentsTable.status, status)).orderBy(desc(kycDocumentsTable.createdAt));
  } else {
    docs = await db.select().from(kycDocumentsTable).orderBy(desc(kycDocumentsTable.createdAt));
  }
...
router.post("/admin/kyc/:id/reject", requireAdmin, async (req, res) => {
  ...
  const [user] = await db.select().from(usersTable).where(eq(usersTable.id, doc.userId)).limit(1);
  if (user) {
    await db.update(usersTable).set({ kycStatus: "rejected" }).where(eq(usersTable.id, user.id));
  }
...
router.post("/admin/plans", requireAdmin, async (req, res) => {
  ...
  const [plan] = await db.insert(plansTable).values({
    name, description, minDeposit: minDeposit.toString(),
    maxDeposit: maxDeposit ? maxDeposit.toString() : null,
    dailyRate: dailyRate.toString(), durationDays, isActive: true,
    features: features ?? [],
  }).returning();
  res.status(201).json({
    id: plan.id, name: plan.name, description: plan.description,
    minDeposit: parseFloat(plan.minDeposit), maxDeposit: plan.maxDeposit ? parseFloat(plan.maxDeposit) : null,
    dailyRate: parseFloat(plan.dailyRate), durationDays: plan.durationDays,
    isActive: plan.isActive, features: plan.features ?? [],
  });
});
...
router.get("/admin/tickets", requireAdmin, async (req, res) => {
  ...
  let tickets;
  if (status) {
    tickets = await db.select().from(ticketsTable).where(eq(ticketsTable.status, status)).orderBy(desc(ticketsTable.updatedAt));
  } else {
    tickets = await db.select().from(ticketsTable).orderBy(desc(ticketsTable.updatedAt));
  }
...
router.post("/admin/tickets/:id/reply", requireAdmin, async (req, res) => {
  ...
  const [msg] = await db.insert(ticketMessagesTable).values({
    ticketId: id, userId: adminUser.id, message: parsed.data.message, isAdmin: "true",
  }).returning();
...
router.post("/admin/tickets/:id/close", requireAdmin, async (req, res) => {
  ...
  res.json({
    ...
    messageCount: msgs.length, createdAt: ticket.createdAt?.toISOString(),
    lastReplyAt: ticket.lastReplyAt?.toISOString() ?? ticket.createdAt?.toISOString(),
  });
});
